...
function webnus_admin_page() {
	...
	if ( version_compare( $theme_version , $version , '<') ){
		$new_version = ' <span class="update-plugins"><span class="update-count">'.__( 'New Version', 'easyweb' ).'</span></span>';
	}else{
		$new_version = '';
	}
...
function webnus_welcome(){
	...
	$version = isset( $verify_inc->get_MEC_info('info')->item->wordpress_theme_metadata->version) ? $verify_inc->get_MEC_info('info')->item->wordpress_theme_metadata->version : '';
					...
					<div class="w-box activation <?php echo $verify_class; ?>">
						<form id="activate_theme_form">
							...
							<div class="w-box-head">
								...
								echo "</span>";
								esc_html_e('Theme Activation','easyweb');
								if ( version_compare( $theme_version , $version , '<') ){
									if( isset( $verify->code ) && !isset( $verify->error ) ) {
										echo '<div class="link-update-theme w-button"> <a href="'. admin_url("update-core.php") .'">' . esc_html__('Click to Update Theme','easyweb') . ' </a> </div>';
									}
								}
								?>
							</div>
							<div class="w-box-content">
								<?php  esc_html_e('Please Active Theme to have Auto Update.' , 'easyweb'); ?>
								<div class="w-system-info">
									<label class="webnus_activation_label" for="security_token">
										<?php  esc_html_e('Seciurty Token','easyweb'); ?>
									</label>
									<?php
								...
								<?php if( isset( $verify->code ) && !isset( $verify->error ) ): ?>
									<div class="w-button">
										<button class="w-button activate_button green" type="submit" id="activate_theme" >
											<?php echo esc_html__('Activated','easyweb'); ?>
										</button>
									</div>
								<?php else: ?>
									<div class="w-button">
										<button class="w-button activate_button" type="submit" id="activate_theme" >
											<?php echo esc_html__('Activate','easyweb'); ?>
										</button>
									</div>
								<?php endif; ?>
							</div>
